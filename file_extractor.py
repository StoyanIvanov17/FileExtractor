...
# We iterate over the selected path above (where we download or store what we want to extract).
# As long as we have only one (the zip file we want to use for example) in that folder \
# we do not need to specify its name each time we run the script.
